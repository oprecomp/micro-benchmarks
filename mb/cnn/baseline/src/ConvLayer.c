...
/**
 *  Allocates a new ConvLayer data structure and its fields (weight, bias,
 *  output feature maps).
 *
 *  @return a pointer to the new ConvLayer data structure.
 *
 *  @param n_out_feat
 ...
 *      NULL, ConvLayer_new() will allocate y automatically.
 */
ConvLayer *ConvLayer_new(
    ...
    // build ConvLayer
    ConvLayer *layer;
    layer = ccn_malloc(sizeof(ConvLayer));
...
static void ConvLayer_pipe_fe(
...
#endif /* FETCH_PROFILE */
    {
        ...
        printf("[ConvLayer %s] Fetch checksum %d,%d,%d,%d: x=%d W=%d\n", layer->name, aa, bb, ii, jj, sum_x, sum_W);
    ...
    printf("[ConvLayer %s] Fetch profiling: %d\n", layer->name, t0);
#endif /* FETCH_PROFILE */

}

static void ConvLayer_pipe_ex(
    ConvLayer *layer,
    int aa,
    int bb,
    int ii,
    ...
    for(int a=0; a<_nof; a++) {
        ...
        {
            int i, sum=0;
            printf("[ConvLayer %s] Intermediate checksum tile %d,%d,%d,%d, a=%d: ", layer->name, aa,bb,ii,jj, print_flag);
            sum=0;
    ...
    {
        int i, sum=0;
        printf("[ConvLayer %s] Tile checksum %d,%d,%d,%d: ", layer->name, aa,bb,ii,jj);
        sum=0;
        for(i=0; i<_nif*_h*_w; i++){
            sum+=_x[i];
        }
    ...
    perf_stop();
    int t0 = perf_get_cycles();
    printf("[ConvLayer %s] Execute profiling: %d\n", layer->name, t0);
#endif /* EXECUTE_PROFILE */

}

static void ConvLayer_pipe_wb(
    ConvLayer *layer,
    int aa,
    int bb,
    int ii,
    ...
    // #pragma omp single
    {
        ...
        if(bb == layer->ntile_nif-1) {
            ...
            printf("[ConvLayer %s] Writeback debug %d,%d,%d,%d:\n", layer->name, aa, bb, ii, jj);
            for(int i=0; i<_nof; i++) {
                for(int j=0; j<_oh; j++) {
                    for(int k=0; k<_ow; k++) {
                        printf("  (%d,%d,%d): %04x\n", i,j,k, layer->loc_y_wb[i*_oh*_ow+j*_ow+k] & 0xffff);
                    }
                }
            }
            ...
            ccn_memcpy_async_3d(
                ...
                _oh,
                _ow*sizeof(data_t),
                layer->height-_fs+1, // remote strides
                (layer->width-_fs+1)*sizeof(data_t),
                _oh, // local strides
                _ow*sizeof(data_t)
            );
#endif /* CCN_TILING_3D */
            ...
            ccn_memcpy_async_2d(
                ...
                layer->loc_y_wb,
                _nof, // sizes
                _oh*_ow*sizeof(data_t),
                (layer->height-_fs+1)*(layer->width-_fs+1)*sizeof(data_t), // local strides
                _oh*_ow*sizeof(data_t) // remote strides
            );
#endif /* CCN_TILING_2D */
#ifdef CCN_TILING_1D
            /* Assuming that tiles are internally contiguous in the i,j feature map dimensions,
    ...
    perf_stop();
    int t0 = perf_get_cycles();
    printf("[ConvLayer %s] Writeback profiling: %d\n", layer->name, t0);
#endif /* WRITEBACK_PROFILE */
}

/**
 *  Executes the given ConvLayer, i.e. computes its outputs given the inputs
 ...
 *
 *  @param *layer
 *      a pointer to the ConvLayer data structure to execute.
 */
void ConvLayer_exec(ConvLayer *layer) {

    // ConvLayer_exec is now organized as a pipeline with the following stages
...
#ifdef PIPE_DEBUG
                    printf("[ConvLayer %s pipe] aa=%d bb=%d ii=%d jj=%d\n", layer->name, aa_pipe, bb_pipe, ii_pipe, jj_pipe);
...
#endif
                            ConvLayer_pipe_fe(layer, aa_fe, bb_fe, ii_fe, jj_fe);

                        // execute stage
#ifndef DISABLE_OPENMP
                        if(omp_get_thread_num() == THREAD_EX)
#endif
                            ConvLayer_pipe_ex(layer, aa_ex, bb_ex, ii_ex, jj_ex);

                        // write-back stage
#ifndef DISABLE_OPENMP
                        if(omp_get_thread_num() == THREAD_WB)
#endif
                            ConvLayer_pipe_wb(layer, aa_wb, bb_wb, ii_wb, jj_wb);
                    }
#ifdef PIPE_PROFILE
                    stop_timer();
                    int t0 = get_time();
                    reset_timer();
                    printf("[ConvLayer %s] Pipe profiling: %d\n", layer->name, t0);
#endif /* PIPE_PROFILE */
...
#else /* ~CCN_TILING */
    // fetch stage
    ConvLayer_pipe_fe(layer, 0, 0, 0, 0);

    // execute stage
    ConvLayer_pipe_ex(layer, 0, 0, 0, 0);

    // write-back stage
    ConvLayer_pipe_wb(layer, 0, 0, 0, 0);
#endif /* CCN_TILING */
